...
**실행기 준비 완료 (2026-07-26)**: `rl_walking/deploy/policy_runner.py` — 관측 45차원 조립·
추론·슬루를 담당하는 단독 실행기 (isaaclab 미의존, ONNX/torch 자동). 시뮬 폐루프에서
관측·목표각이 학습 파이프라인과 **오차 0.00e+00 (비트 동일)** 검증됨
(log_picture/실행기_폐루프검증.txt). 사용법·센서 매핑: `rl_walking/deploy/README.md`.
1. 실물 로봇을 손으로 세워두고 iAHRS+엔코더 로그만 수집 (송신 없음)
2. 로그를 PolicyRunner.step()에 입력해 출력 목표각의 타당성 검사 (발산·리밋 위반·슬루 포화율)
...
- **대역폭 검증 완료 (2026-07-26, 목 벤치 — log_picture/대역폭검증_요약.txt)**:
  600msg/s 여유 PASS (데드라인 미스 0, 예산 26%). 노드는 세트포인트 구조라
  CAN 송신은 입력율 무관 50Hz×12 고정 — 멀티관절 명령은 성능상 필수 아님.
  단 개발 PC 기준 측정이므로 Jetson 배포 시 동일 벤치 재실행 권장.
  (선택 개선: 명령당 상태에코 제거·QoS depth 상향 — 요약 문서 참조)

...
- 1차 험지 런(2500 iter, 3단계 웜스타트): **커리큘럼 레벨 2.6/10 도달** ≈ 단높이 2~3.5cm 구간 정복 (log_picture/14 영상 — 계단 위 보행), 페이로드·지연 조건 유지한 채
- 정직한 평가: 6cm까지는 미도달 — 연장 학습(+5000 iter) 진행분은 `logs/.../` 최신 런 참조. 블라인드(지형 센서 없음) 한계도 있어, 압력센서 관측 추가·D455 높이맵이 다음 단계
- 시뮬 평균 생존 13~15s/20s (혼합 지형 + 배터리 + 지연 동시 조건)
